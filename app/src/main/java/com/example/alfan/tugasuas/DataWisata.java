package com.example.alfan.tugasuas;

import android.Manifest;
import android.content.Intent;
import android.content.pm.PackageManager;
import android.graphics.Bitmap;
import android.graphics.BitmapFactory;
import android.graphics.drawable.BitmapDrawable;
import android.net.Uri;
import android.support.annotation.NonNull;
import android.support.v4.app.ActivityCompat;
import android.support.v7.app.AppCompatActivity;
import android.os.Bundle;
import android.view.View;
import android.widget.ArrayAdapter;
import android.widget.Button;
import android.widget.EditText;
import android.widget.ImageView;
import android.widget.Spinner;
import android.widget.TextView;
import android.widget.Toast;

import java.io.ByteArrayOutputStream;
import java.io.FileNotFoundException;
import java.io.InputStream;

public class DataWisata extends AppCompatActivity {
    String kategori[] = {"-- Pilih Kategori --","Pantai","Gunung","Musium","AirTerjun","Taman"};
    Spinner datakategori;

    EditText edtjudul, edtalamat,edtpin,edtdeskripsi;
    Button btnChoose, btnAdd, btnList;
    ImageView imageView;
    String hasilKategori;

    TextView textkategori;

    final int REQUEST_CODE_GALLERY = 999;

    public static SQLiteHelper sqLiteHelper;


    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_data_wisata);
        sqLiteHelper = new SQLiteHelper(this, "wisatadb", null, 1);

        sqLiteHelper.queryData("CREATE TABLE IF NOT EXISTS wisata(Id INTEGER PRIMARY KEY AUTOINCREMENT, judul VARCHAR NOT NULL, alamat VARCHAR, pin VARCHAR,deskripsi VARCHAR,kategori VARCHAR,image BLOB )");

        datakategori = (Spinner) findViewById(R.id.kategori);

        ArrayAdapter<String> arrayKategori = new ArrayAdapter<>(this, R.layout.activity_kategori, R.id.textkategori, kategori);
        datakategori.setAdapter(arrayKategori);

        edtjudul = (EditText) findViewById(R.id.judul);
        edtalamat = (EditText) findViewById(R.id.alamat);
        edtpin = (EditText) findViewById(R.id.pin);
        edtdeskripsi = (EditText) findViewById(R.id.deskripsi);
        datakategori = (Spinner) findViewById(R.id.kategori);
        btnChoose = (Button) findViewById(R.id.btnChoose);
        btnAdd = (Button) findViewById(R.id.btnAdd);
        btnList = (Button) findViewById(R.id.btnList);
        imageView = (ImageView) findViewById(R.id.imageView);
        textkategori = (TextView) findViewById(R.id.textkategori);



        btnChoose.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                ActivityCompat.requestPermissions(
                        DataWisata.this,
                        new String[]{Manifest.permission.READ_EXTERNAL_STORAGE},
                        REQUEST_CODE_GALLERY
                );
            }
        });
        btnAdd.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {

                String prodi = datakategori.getSelectedItem().toString();
                try{
                    sqLiteHelper.insertData(
                            edtjudul.getText().toString().trim(),
                            edtalamat.getText().toString().trim(),
                            edtpin.getText().toString().trim(),
                            edtdeskripsi.getText().toString().trim(),
                            prodi,
                            imageViewToByte(imageView)

                    );
                    Toast.makeText(getApplicationContext(), "Added successfully!", Toast.LENGTH_SHORT).show();

                }
                catch (Exception e){
                    Toast.makeText(getApplicationContext(), "Error!", Toast.LENGTH_SHORT).show();
                }
            }
        });
        btnList.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(DataWisata.this, WisataList.class);
                startActivity(intent);
            }
        });


    }
    public static byte[] imageViewToByte(ImageView image) {
        Bitmap bitmap = ((BitmapDrawable)image.getDrawable()).getBitmap();
        ByteArrayOutputStream stream = new ByteArrayOutputStream();
        bitmap.compress(Bitmap.CompressFormat.PNG, 100, stream);
        byte[] byteArray = stream.toByteArray();
        return byteArray;
    }
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {

        if(requestCode == REQUEST_CODE_GALLERY){
            if(grantResults.length >0 && grantResults[0] == PackageManager.PERMISSION_GRANTED){
                Intent intent = new Intent(Intent.ACTION_PICK);
                intent.setType("image/*");
                startActivityForResult(intent, REQUEST_CODE_GALLERY);
            }
            else {
                Toast.makeText(getApplicationContext(), "You don't have permission to access file location!", Toast.LENGTH_SHORT).show();
            }
            return;
        }

        super.onRequestPermissionsResult(requestCode, permissions, grantResults);
    }
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {

        if(requestCode == REQUEST_CODE_GALLERY && resultCode == RESULT_OK && data != null){
            Uri uri = data.getData();

            try {
                InputStream inputStream = getContentResolver().openInputStream(uri);

                Bitmap bitmap = BitmapFactory.decodeStream(inputStream);
                imageView.setImageBitmap(bitmap);

            } catch (FileNotFoundException e) {
                e.printStackTrace();
            }
        }

        super.onActivityResult(requestCode, resultCode, data);
    }


}
